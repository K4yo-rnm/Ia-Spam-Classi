...
async function carregarConfiguracoes() {
  ...
  try {
    const resposta = await fetch("http://127.0.0.1:5000/configuracoes");
    const resultado = await resposta.json();
...
async function analisarNovosEmails() {
  ...
  if (analiseEmAndamento) {
    console.log("analise já está em andamento. Ignorando nova chamada.");
    return;
  }
  ...
  if (!email || !senha) {
    if (mensagem) {
      mensagem.textContent = "Configuração de e-mail incompleta. Conecte o e-mail novamente.";
      mensagem.style.color = "red";
    }
  ...
  try {
    ...
    if (mensagem) {
      mensagem.textContent = "Buscando e analisando novos e-mails...";
      mensagem.style.color = "#111827";
    }

    const resposta = await fetch("http://127.0.0.1:5000/analisar-emails", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
    ...
    if (mensagem) {
      mensagem.textContent =
        `Análise concluída: ${resultado.novos_analisados} novos e-mails analisados e ${resultado.repetidos_ignorados} repetidos ignorados.`;
